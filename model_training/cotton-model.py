...
train=os.path.abspath(os.path.join(current,'..','data','cotton','data','train'))
val=os.path.abspath(os.path.join(current,'..','data','cotton','data','val'))
test=os.path.abspath(os.path.join(current,'..','data','cotton','data','test'))
img_size=224 #fix input image size for vgg16 224x224
batch_size=32 #32 images are passed to gpu for training
epochs=10 #dataset is passed 10 times for training
...
test_loader=DataLoader(test_dataset,batch_size=batch_size,shuffle=False)#shuffle not needed
# ==================== VGG16 BASE MODEL ====================
...
# ==================== VALIDATION ====================
results=os.path.abspath(os.path.join(current,'..','results','cotton'))#to save model's data 
...
torch.save(model.state_dict(),os.path.join(save,"cotton.pth"))#save model
